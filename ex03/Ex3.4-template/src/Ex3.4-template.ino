...
// (e.g. desk lamp close to sensor). Increase this if lightPct stays stuck near 0.
...
const char* stateToString(SystemState s) {
    switch (s) {
        ...
        case STATE_STRESSED:  return "STRESSED";
...
void loop() {
    ...
    if (!inWarmUp && (now - lastScoreUpdate >= SCORE_INTERVAL)) {
        ...
        healthScore = 0;
        if (lightPct >= 25 && lightPct <= 90) healthScore += 25;   // Ex3.1 range
        if (temp  >= 18.0f && temp  <= 30.0f) healthScore += 25;
        if (humid >= 30.0f && humid <= 75.0f) healthScore += 25;
        if (eCO2 < 1200)                       healthScore += 25;
    ...
    if (now - lastBleTransmission >= BLE_INTERVAL) {
        ...
        char buf[64];
        snprintf(buf, sizeof(buf), "T=%.1f H=%.0f L=%d C=%u S=%s",
                 temp, humid, lightPct, eCO2, stateToString(currentState));
